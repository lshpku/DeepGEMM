...
// NOTES: `A`/`SFA`/`D` use the whole contiguous (128-aligned) unzipped layout, and the task
//        descriptor tells the kernel which row range and which expert to use
// NOTES: `SFA` must be MN-major over the whole `M`, exactly as the non-chunk kernels want it,
//        because the TMA descriptor bakes in one single row stride for all the chunks
static void sm100_fp8_chunk_gemm_1d1d(const torch::Tensor& a, const torch::Tensor& sfa,
                                      const torch::Tensor& b, const torch::Tensor& sfb,
                                      const torch::Tensor& d,
    ...
    const auto code = SM100FP8FP4Gemm1D1DRuntime::generate(args);
    const auto runtime = compiler->build("sm100_fp8_chunk_gemm_1d1d", code);
    SM100FP8FP4Gemm1D1DRuntime::launch(runtime, args);
}
