...
CUTLASS_DEVICE void st_rel(const int* ptr, const int& value) {
    asm volatile("st.release.gpu.global.s32 [%0], %1;" :: "l"(ptr), "r"(value));
}
...
CUTLASS_DEVICE int atomic_add(const int* ptr, const int& value) {
    int ret;
    asm volatile("atom.gpu.global.add.s32 %0, [%1], %2;" : "=r"(ret) : "l"(ptr), "r"(value));
    return ret;
}
